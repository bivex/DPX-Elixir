"""Typer CLI interface for DPX-Elixir."""
...
app = typer.Typer(
    name="dpx-elixir",
    help="💧 Hexagonal Pattern Scanner & Architecture Detector for Elixir / OTP (Elixir 1.14 - 1.18+).",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to an Elixir source file (.ex / .exs) or directory containing an Elixir application / umbrella project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'gen_server', 'supervisor', 'pipeline_operator', 'protocol_polymorphism').",
        ),
    ] = None,
    ...
    """Scan an Elixir project or source file for OTP behaviours, actor concurrency, and resilience smells."""
...
def _handle_terminal_scan(
    ...
    # 1. Header Banner
    header = Panel.fit(
        f"[bold white]💧 DPX-Elixir: OTP Pattern Scanner & Architecture Detector • Hexagonal DDD[/bold white]\n"
    ...
    for idx, d in enumerate(report.detections, 1):
        ...
        header_text = f"[bold magenta]#{idx} {d.pattern_type.value.upper()}[/bold magenta] on [bold]{d.target_kind}[/bold] '[yellow]{d.target_name}[/yellow]'"
        ...
        for i, ev in enumerate(d.evidences):
            ...
            lines.append(f"{connector} [magenta]+{int(ev.weight * 100)}%[/magenta] ([dim]{ev.rule_code}[/dim]) {ev.description}{ev_loc}")
...
@app.command()
def rules() -> None:
    """List all supported Elixir / OTP design patterns, behaviours, and resilience rules."""
    table = Table(title="💧 Supported Patterns & Rules in DPX-Elixir", header_style="bold magenta")
...
@app.command()
def info() -> None:
    """Display DPX-Elixir architecture, version, and grammar engine details."""
    panel = Panel(
        """[bold magenta]💧 DPX-Elixir • OTP Pattern Scanner & Architecture Detector[/bold magenta]
[dim]Multi-Paradigm Hexagonal Pattern Detector & Elixir AST Engine[/dim]

• [bold]Target Language:[/bold] Elixir / OTP (Elixir 1.14 - 1.18+ edition)
• [bold]Architecture:[/bold] Hexagonal Ports & Adapters + Domain-Driven Design (DDD)
• [bold]Rules Supported:[/bold] 30 Rules (GenServer, Supervisor, DynamicSupervisor, Agent, Protocol, Plug, Phoenix Context, Let-It-Crash)
