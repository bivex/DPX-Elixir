"""OASIS SARIF v2.1.0 Report Formatter for Elixir / OTP Pattern Detector."""
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport) -> str:
        ...
        for d in report.detections:
            rule_id = f"DPX-ELIXIR-{d.pattern_type.value.upper()}"
            if rule_id not in rules_map:
                rules_map[rule_id] = {
                    "id": rule_id,
                    "name": d.pattern_type.value,
                    "shortDescription": {"text": f"Elixir Pattern: {d.pattern_type.value.replace('_', ' ').title()}"},
